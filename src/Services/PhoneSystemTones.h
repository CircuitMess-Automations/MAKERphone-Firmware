...
public:
	...
	/**
	 * S259 - derived per-rest-step minimum-duration accessor for
	 * chime `id`. Returns the shortest catalogued rest-step
	 * durationMs value the engine holds the piezo SILENT for, in
	 * ms. Where `restDurationMs(id)` (S247) reports the SUM of the
	 * catalogued rest-step durations, `restNoteCount(id)` (S244)
	 ...
	 * (S258) reports the per-step CEILING, `troughRestDurationMs(id)`
	 * reports the per-step FLOOR -- the rest-axis sibling of
	 * `troughNoteDurationMs(id)` (S251) on the audible-step axis,
	 * exactly how `peakRestDurationMs(id)` (S258) is the rest-axis
	 * sibling of `peakNoteDurationMs(id)` (S250) and
	 * `meanRestDurationMs(id)` (S257) is the rest-axis sibling of
	 * `meanNoteDurationMs(id)` (S249) on the audible-step axis.
	 * Closes the rest-axis (CEILING, FLOOR, CENTRE) trio
	 ...
	 * `troughNoteDurationMs`, `meanNoteDurationMs`).
	 *
	 * So a future "Settings -> Sounds -> System chimes" picker row
	 * caption like "(shortest rest 40 ms)" or a future
	 * `PhoneDiagScreen` chime-roster footer line that wants to
	 * render the shortest per-rest hold of a cue can read a
	 * dedicated derived accessor for the per-rest-step FLOOR
	 * instead of scanning the catalogued `Note*` array at the call
	 * site. Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, and for the no-rests-melody
	 * case (where `restNoteCount(id) == 0` and no rest step exists,
	 * so the FLOOR is undefined) -- so a flat audible-only cue
	 * with no rests, or an out-of-range id, collapses to the same 0
	 * the catalogued rest-axis pair `restNoteCount` (S244) /
	 * `restDurationMs` (S247), the rest-axis CENTRE
	 * `meanRestDurationMs` (S257), and the rest-axis CEILING
	 * `peakRestDurationMs` (S258) already collapse to.
	 *
	 * Saturates at `0xFFFF` ms (the same uint16_t ceiling the
	 * duration cluster `durationMs` / `audibleDurationMs` /
	 * `restDurationMs` / `gapTotalMs` / `meanNoteDurationMs` /
	 ...
	 * the cap is in practice unreachable, but the saturate-on-
	 * overflow guard keeps the return type honest.
	 *
	 * Distinct from `restDurationMs` (rest-step SUM in isolation,
	 * not per-step FLOOR), distinct from `restNoteCount`
	 * (rest-step COUNT in isolation, not per-step FLOOR), distinct
	 * from `meanRestDurationMs` (rest-axis per-step CENTRE, not
	 * per-step FLOOR), distinct from `peakRestDurationMs`
	 * (rest-axis per-step CEILING, not per-step FLOOR), distinct
	 * from `troughNoteDurationMs` (audible-axis per-step FLOOR,
	 * not rest-axis FLOOR), distinct from `peakNoteDurationMs`
	 * (audible-axis per-step CEILING, not rest-axis FLOOR),
	 * distinct from `meanNoteDurationMs` (audible-axis per-step
	 * CENTRE, not rest-axis FLOOR), distinct from
	 * `audibleDurationMs` (audible-step SUM in isolation),
	 * distinct from `durationMs` (TOTAL incl. audible + rests +
	 * gaps), distinct from `gapTotalMs` (inter-step gap component
	 * in aggregate, not per-rest FLOOR), distinct from `gapMs`
	 * (per-step inter-step filler in isolation, not per-rest
	 * FLOOR), distinct from `firstNoteDurationMs` /
	 * `lastNoteDurationMs` (structural endpoints on the audible-
	 * duration axis, not rest-axis FLOOR), distinct from
	 * `durationSpanMs` / `audibleDurationSpanMs` (derived endpoint
	 * / envelope MAGNITUDES on the audible-duration axis, not
	 * rest-axis FLOOR), and distinct from
	 * `PhoneRingtoneEngine::isPlaying()` / `currentFreq()` (the
	 * S191 live-piezo accessors that report runtime playback
	 * state, not catalogued shape). Profile-state INDEPENDENT: the
	 * catalogued per-rest-step FLOOR is the same on SILENT /
	 * MEETING profiles as on GENERAL / OUTDOOR / HEADSET (the
	 * S231 `tryPlay(id)` gate already reports the silenced answer
	 * separately for any caller that wants to fade the row caption
	 * into a "(silenced)" form).
	 *
	 * Cheap O(notes) linear scan with a single uint16_t running
	 * min guarded by a `found` sentinel (no natural starting value
	 * for a min-search across uint16_t when the rest set may be
	 * empty) -- mirroring `troughNoteDurationMs(id)` (S251)
	 * exactly with the rest-step predicate (`freq == 0`)
	 * substituted for the audible-step predicate (`freq != 0`),
	 * and mirroring `peakRestDurationMs(id)` (S258) exactly with
	 * `<` substituted for `>`. No per-call allocation, no
	 * recursion into the existing `restDurationMs` /
	 * `restNoteCount` / `meanRestDurationMs` /
	 * `peakRestDurationMs` accessors. Header surface grows by
	 * exactly one public symbol (`static uint16_t
	 ...
	 * existing call site of the catalogue keeps byte-identical
	 * behaviour -- the new helper is purely additive. Closes the
	 * rest-axis (CEILING, FLOOR, CENTRE) trio next to the audible-
	 * axis (CEILING, FLOOR, CENTRE) trio already on the duration
	 * axis; the natural follow-up is the rest-axis MAGNITUDE
	 * (`restDurationSpanMs = peakRestDurationMs -
	 * troughRestDurationMs`) that mirrors `audibleDurationSpanMs`
	 * (S256) on the audible-duration axis.
	 */
	static uint16_t troughRestDurationMs(uint8_t id);

	/**
	 * S260 - derived rest-CEILING-FLOOR magnitude accessor on the
	 * DURATION axis. Returns the unsigned absolute difference, in
	 * milliseconds, between the catalogued per-rest-step CEILING
	 * note duration and the catalogued per-rest-step FLOOR note
	 * duration of the underlying Melody --
	 * `peakRestDurationMs(id) - troughRestDurationMs(id)` (always
	 * non-negative by construction, since `peakRestDurationMs` is
	 * a max-search and `troughRestDurationMs` is a min-search
	 * across the same rest-step subset, so `peak >= trough` is
	 * invariant). The rest-axis sibling of `audibleDurationSpanMs`
	 * (S256) on the audible-duration envelope axis: where
	 * `audibleDurationSpanMs(id)` reports the magnitude of the
	 * (CEILING, FLOOR) AUDIBLE envelope so a caller can render an
	 * audible-duration-range tick at the full envelope of the
	 * audible steps, `restDurationSpanMs(id)` reports the
	 * magnitude of the (CEILING, FLOOR) REST envelope so the same
	 * caller can render a rest-duration-range tick at the full
	 * envelope of the silent steps. Builds directly on the S258 /
	 * S259 pair (`peakRestDurationMs` / `troughRestDurationMs`)
	 * that closed the rest-CEILING / FLOOR pair on the duration
	 * axis; S260 promotes that pair to a derived MAGNITUDE
	 * accessor in the same way S256 promoted the S250 / S251
	 * audible CEILING / FLOOR pair (`peakNoteDurationMs` /
	 * `troughNoteDurationMs`) to `audibleDurationSpanMs`, and in
	 * the same way S254 promoted the S252 / S253 endpoint pair
	 * (`firstNoteDurationMs` / `lastNoteDurationMs`) to
	 * `durationSpanMs`, S239 promoted (`firstFreqHz` /
	 * `lastFreqHz`) to `pitchSpanHz`, and S255 promoted
	 * (`peakFreqHz` / `troughFreqHz`) to `audiblePitchSpanHz`.
	 * The duration-axis structural / audible / rest / derived
	 * shape now matches the pitch-axis shape on the rest side too:
	 * `peakRestDurationMs` / `troughRestDurationMs` (S258 / S259)
	 * at the rest CEILING / FLOOR, and now `restDurationSpanMs`
	 * (S260) at the rest CEILING / FLOOR magnitude.
	 *
	 * So a future `PhoneDiagScreen` "Sound test" walk that wants
	 * to render the rest-duration range of a cue (e.g. the full
	 * horizontal extent of a rest-duration-bar between longest-
	 * rest tick and shortest-rest tick) or a future "Settings ->
	 * Sounds -> System chimes" picker row caption like
	 * "(rest spans 200 ms)" can read a dedicated derived
	 * accessor for the rest-axis envelope magnitude instead of
	 * computing `peakRestDurationMs(id) -
	 * troughRestDurationMs(id)` at the call site. Returns 0 for
	 * an out-of-range id (collapsing transparently because both
	 * `peakRestDurationMs(id)` and `troughRestDurationMs(id)`
	 * already collapse to 0 there), for the (currently
	 * impossible) empty-melody case, for the no-rests-melody case
	 * (both sides collapse to 0 there too), and naturally for any
	 * flat-rest-duration chime whose rest CEILING and FLOOR
	 * coincide.
	 *
	 * Saturates at `0xFFFF` ms (the same uint16_t ceiling the
	 * duration cluster `durationMs` / `audibleDurationMs` /
	 ...
	 * `troughRestDurationMs` already share); since both inputs
	 * are themselves uint16_t and the subtraction is always non-
	 * negative the cap is in practice unreachable.
	 *
	 * Distinct from `audibleDurationSpanMs` (audible envelope
	 * magnitude on the duration axis, not rest envelope
	 * magnitude), distinct from `durationSpanMs` (structural
	 * endpoint magnitude on the duration axis, not rest CEILING /
	 * FLOOR magnitude), distinct from `peakRestDurationMs` /
	 * `troughRestDurationMs` (the catalogued rest CEILING / FLOOR
	 * themselves, not the magnitude between them), distinct from
	 * `meanRestDurationMs` (rest-axis per-step CENTRE, not
	 * envelope magnitude), distinct from `peakNoteDurationMs` /
	 * `troughNoteDurationMs` (audible-axis per-step extents),
	 * distinct from `meanNoteDurationMs` (audible-step CENTRE),
	 * distinct from `firstNoteDurationMs` / `lastNoteDurationMs`
	 * (structural endpoints on the audible-duration axis),
	 * distinct from `audiblePitchSpanHz` / `pitchSpanHz` (pitch
	 * axis magnitudes, not duration axis), distinct from
	 * `restDurationMs` (rest-step duration TOTAL, not envelope
	 * magnitude), distinct from `audibleDurationMs` (audible-step
	 * duration TOTAL), distinct from `gapTotalMs` / `gapMs`
	 * (inter-step filler component, not per-rest envelope
	 * magnitude), distinct from `silhouette` (signed tilt sign,
	 * not absolute rest magnitude). Profile-state INDEPENDENT:
	 * the catalogued rest envelope magnitude is the same on
	 * SILENT / MEETING profiles as on GENERAL / OUTDOOR /
	 * HEADSET (the S231 `tryPlay(id)` gate already reports the
	 * silenced answer separately for any caller that wants to
	 * fade the row caption into a "(silenced)" form).
	 *
	 * Cheap O(notes): two linear scans (max + min, mirroring the
	 * S258 / S259 implementations exactly) plus one subtraction;
	 * no engine interaction, no persisted state, no per-call
	 * allocation; mirrors `audibleDurationSpanMs(id)` (S256)
	 * exactly with `peakRestDurationMs` / `troughRestDurationMs`
	 * substituted for `peakNoteDurationMs` /
	 * `troughNoteDurationMs`. Delegates straight to the S258 /
	 * S259 CEILING / FLOOR pair so the catalogued `Note*` array
	 * is walked twice per call (once for the max, once for the
	 * min), exactly matching the `audibleDurationSpanMs` cost
	 * model. Header surface grows by exactly one public symbol
	 * (`static uint16_t restDurationSpanMs`); the cpp adds a
	 * single function next to the existing `count` / `valid` /
	 * `name` / `melody` / `play` / `tryPlay` / `isSilenced` /
	 * `durationMs` / `noteCount` / `firstFreqHz` / `lastFreqHz`
	 * / `gapMs` / `loops` / `silhouette` / `pitchSpanHz` /
	 ...
	 * `peakRestDurationMs` / `troughRestDurationMs` cluster. No
	 * new includes, no new const data, no new SPIFFS asset cost.
	 * Every existing call site of the catalogue keeps byte-
	 * identical behaviour -- the new helper is purely additive.
	 * Closes the rest-axis envelope-magnitude corner at full
	 * symmetry with the audible-axis envelope-magnitude corner
	 * (`audibleDurationSpanMs`, S256) on the duration axis,
	 * completing the (audible-envelope, rest-envelope) derived-
	 * magnitude pair on the duration side that mirrors the
	 * (audible-envelope, structural-endpoint) pair the pitch axis
	 * already covers via `audiblePitchSpanHz` (S255) and
	 * `pitchSpanHz` (S239).
	 */
	static uint16_t restDurationSpanMs(uint8_t id);

	/**
	 * S261 -- Structural first-rest duration accessor for chime
	 * `id`. Returns the catalogued duration in ms of the FIRST
	 * `PhoneRingtoneEngine::Note` entry whose `freq == 0` (i.e.
	 * the LEADING rest-step) in the underlying Melody. The
	 * rest-axis sibling of `firstNoteDurationMs(id)` (S252) on
	 * the structural-endpoint axis: where `firstNoteDurationMs`
	 * walks the catalogued array LEADING endpoint regardless of
	 * note-vs-rest character and effectively reports the
	 * structural LEADING duration, `firstRestDurationMs` walks
	 * the catalogued array for the FIRST occurrence of a rest-
	 * step and reports the catalogued duration the engine holds
	 * silence for at that LEADING rest position. Together with
	 * the future `lastRestDurationMs` it would close the rest-
	 * axis structural endpoint pair (LEADING, TRAILING) in the
	 * same way `firstNoteDurationMs` / `lastNoteDurationMs`
	 * (S252 / S253) closed the audible-axis structural endpoint
	 * pair on the duration axis, and in the same way
	 * `firstFreqHz` / `lastFreqHz` (S234 / S235) closed the
	 * structural endpoint pair on the pitch axis.
	 *
	 * Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, for the no-rests-melody
	 * case (a chime that contains zero `freq == 0` steps), and
	 * naturally for any rest step the catalogue marks with a
	 * zero duration. Profile-state INDEPENDENT: the catalogued
	 * leading-rest duration is the same on SILENT / MEETING
	 ...
	 * / `troughRestDurationMs` (rest CEILING / FLOOR across
	 * every rest, not the LEADING rest), distinct from
	 * `restDurationSpanMs` (rest CEILING-FLOOR magnitude, not
	 * the LEADING rest), distinct from `restNoteCount` (rest-
	 * step COUNT, not the LEADING rest's duration).
	 *
	 * Cheap O(notes): a single forward linear scan that returns
	 * on the first `freq == 0` hit; no engine interaction, no
	 * persisted state, no per-call allocation. Header surface
	 * grows by exactly one public symbol (`static uint16_t
	 * firstRestDurationMs`); the cpp adds a single function next
	 * to the existing `count` / `valid` / `name` / `melody` /
	 * `play` / `tryPlay` / `isSilenced` / `durationMs` /
	 * `noteCount` / `firstFreqHz` / `lastFreqHz` / `gapMs` /
	 * `loops` / `silhouette` / `pitchSpanHz` / `peakFreqHz` /
	 ...
	 * `audiblePitchSpanHz` / `audibleDurationSpanMs` /
	 * `meanRestDurationMs` / `peakRestDurationMs` /
	 * `troughRestDurationMs` / `restDurationSpanMs` cluster. No
	 * new includes, no new const data, no new SPIFFS asset
	 * cost. Every existing call site of the catalogue keeps
	 * byte-identical behaviour -- the new helper is purely
	 * additive.
	 */
	static uint16_t firstRestDurationMs(uint8_t id);

	/**
	 * S262 -- Structural last-rest duration accessor for chime
	 * `id`. Returns the catalogued duration in ms of the LAST
	 * `PhoneRingtoneEngine::Note` entry whose `freq == 0` (i.e.
	 * the TRAILING rest-step) in the underlying Melody. The
	 * trailing-endpoint sibling of `firstRestDurationMs(id)`
	 * (S261) on the rest-axis structural-endpoint corner, and
	 * the rest-axis analogue of `lastNoteDurationMs(id)` (S253)
	 * on the audible-axis structural-endpoint corner: where
	 * `lastNoteDurationMs(id)` reads the catalogued TRAILING
	 * duration regardless of note-vs-rest character,
	 * `lastRestDurationMs(id)` scans backward for the LAST
	 * occurrence of a rest-step and reports the catalogued
	 * duration the engine holds silence for at that TRAILING
	 * rest position. With S261 it closes the rest-axis
	 * structural endpoint pair (LEADING, TRAILING) in the same
	 * way `firstNoteDurationMs` / `lastNoteDurationMs` (S252 /
	 * S253) closed the audible-axis structural endpoint pair on
	 * the duration axis, and in the same way `firstFreqHz` /
	 * `lastFreqHz` (S234 / S235) closed the structural endpoint
	 * pair on the pitch axis.
	 *
	 * Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, for the no-rests-melody
	 * case (a chime that contains zero `freq == 0` steps), and
	 * naturally for any rest step the catalogue marks with a
	 * zero duration -- the same "no answer" cases the rest-axis
	 * sibling `firstRestDurationMs(id)` (S261) already collapses
	 * to 0. Profile-state INDEPENDENT: the catalogued trailing-
	 * rest duration is the same on SILENT / MEETING profiles as
	 * on GENERAL / OUTDOOR / HEADSET. Distinct from
	 ...
	 * comes last), distinct from `restDurationMs` (rest-step
	 * SUM across the whole melody, not the TRAILING rest's
	 * duration), distinct from `meanRestDurationMs` (rest-step
	 * CENTRE across every rest, not the TRAILING rest), distinct
	 * from `peakRestDurationMs` / `troughRestDurationMs` (rest
	 * CEILING / FLOOR across every rest, not the TRAILING rest),
	 * distinct from `restDurationSpanMs` (rest CEILING-FLOOR
	 * magnitude, not the TRAILING rest), distinct from
	 * `restNoteCount` (rest-step COUNT, not the TRAILING rest's
	 * duration).
	 *
	 * Cheap O(notes): a single reverse linear scan that returns
	 * on the first `freq == 0` hit from the trailing end; no
	 * engine interaction, no persisted state, no per-call
	 * allocation; mirrors `firstRestDurationMs(id)` (S261)
	 * exactly with the scan direction flipped. Header surface
	 * grows by exactly one public symbol (`static uint16_t
	 * lastRestDurationMs`); the cpp adds a single function next
	 * to the existing `count` / `valid` / `name` / `melody` /
	 * `play` / `tryPlay` / `isSilenced` / `durationMs` /
	 * `noteCount` / `firstFreqHz` / `lastFreqHz` / `gapMs` /
	 * `loops` / `silhouette` / `pitchSpanHz` / `peakFreqHz` /
	 ...
	 * `firstRestDurationMs` cluster. No new includes, no new
	 * const data, no new SPIFFS asset cost. Every existing call
	 * site of the catalogue keeps byte-identical behaviour --
	 * the new helper is purely additive. Closes the rest-axis
	 * structural endpoint pair on the duration axis; the natural
	 * follow-up is a rest-axis structural-endpoint MAGNITUDE
	 * accessor (`restDurationEndpointSpanMs` -- the
	 * `|firstRestDurationMs(id) - lastRestDurationMs(id)|`
	 * derived sibling of `durationSpanMs(id)` (S254) on the
	 * rest-axis structural-endpoint corner).
	 */
	static uint16_t lastRestDurationMs(uint8_t id);

	/**
	 * S263 -- Derived rest-axis structural-endpoint magnitude
	 * accessor on the DURATION axis for chime `id`. Returns the
	 * unsigned absolute difference, in ms, between the
	 * catalogued LEADING rest-step duration and the catalogued
	 * TRAILING rest-step duration of the underlying Melody --
	 * `|firstRestDurationMs(id) - lastRestDurationMs(id)|`. The
	 * rest-axis structural-endpoint sibling of
	 * `durationSpanMs(id)` (S254) on the duration axis: where
	 * `durationSpanMs` reports the magnitude of the (LEADING,
	 * TRAILING) audible-or-rest structural endpoint pair in ms,
	 * `restDurationEndpointSpanMs` reports the magnitude of the
	 * (LEADING, TRAILING) REST-only structural endpoint pair in
	 * ms, so a future PhoneDiagScreen "Sound test" walk that
	 * wants to render the silence-bookend duration delta of a
	 * cue (how much longer or shorter the trailing silence is
	 * than the leading silence) or a future "Settings -> Sounds
	 * -> System chimes" picker row caption like "(rest delta
	 * 40 ms)" can read a dedicated derived accessor for the
	 * rest-axis structural-endpoint magnitude instead of
	 * computing `|firstRestDurationMs(id) - lastRestDurationMs(id)|`
	 * at the call site.
	 *
	 * Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, for the no-rests-melody
	 * case (both endpoint accessors collapse to 0), and
	 * naturally for any chime whose LEADING and TRAILING rest-
	 * step share an identical catalogued duration (including
	 * the structural-degenerate case of a chime with exactly
	 * one rest-step, where the LEADING rest IS the TRAILING
	 * rest and the two accessors return the same value).
	 * Profile-state INDEPENDENT: the catalogued rest-endpoint
	 * delta is the same on SILENT / MEETING profiles as on
	 * GENERAL / OUTDOOR / HEADSET. Distinct from
	 * `durationSpanMs` (structural endpoint magnitude --
	 * audible-or-rest, not REST-only), distinct from
	 * `audibleDurationSpanMs` (audible-axis CEILING-FLOOR
	 * magnitude, not REST-axis endpoint magnitude), distinct
	 * from `restDurationSpanMs` (rest-axis CEILING-FLOOR
	 * magnitude across every rest, not the LEADING-TRAILING
	 * endpoint pair), distinct from `firstRestDurationMs` /
	 * `lastRestDurationMs` (rest-axis endpoint values
	 * themselves, not their unsigned difference).
	 *
	 * Cheap O(notes): two linear scans (one forward, one
	 * reverse) plus a single unsigned subtraction; no engine
	 * interaction, no persisted state, no per-call allocation.
	 * Header surface grows by exactly one public symbol
	 * (`static uint16_t restDurationEndpointSpanMs`); the cpp
	 * adds a single function next to the existing `count` /
	 * `valid` / `name` / `melody` / `play` / `tryPlay` /
	 * `isSilenced` / `durationMs` / `noteCount` / `firstFreqHz`
	 * / `lastFreqHz` / `gapMs` / `loops` / `silhouette` /
	 ...
	 * `meanRestDurationMs` / `peakRestDurationMs` /
	 * `troughRestDurationMs` / `restDurationSpanMs` /
	 * `firstRestDurationMs` / `lastRestDurationMs` cluster. No
	 * new includes, no new const data, no new SPIFFS asset
	 * cost. Every existing call site of the catalogue keeps
	 * byte-identical behaviour -- the new helper is purely
	 * additive. Together with `durationSpanMs(id)` (S254) it
	 * completes the (audible-endpoint, rest-endpoint) derived
	...
	/**
	 * S267 -- Derived cross-axis structural-endpoint MAGNITUDE-of-
	 * MAGNITUDES accessor on the DURATION axis for chime `id`.
	 * Returns the arithmetic mean, in ms, of the audible-axis
	 * structural-endpoint MAGNITUDE (`durationSpanMs(id)`, S254)
	 * and the rest-axis structural-endpoint MAGNITUDE
	 * (`restDurationEndpointSpanMs(id)`, S263) of the underlying
	 * Melody -- `(durationSpanMs(id) +
	 * restDurationEndpointSpanMs(id)) / 2`. The cross-axis
	 * collapse of the (audible-endpoint MAGNITUDE, rest-endpoint
	 * MAGNITUDE) duration-axis pair into a single MAGNITUDE-of-
	 * MAGNITUDES scalar, the MAGNITUDE-axis sibling of
	 * `meanEndpointDurationMs(id)` (S266) which collapses the
	 * CENTRE pair. Closes the (MAGNITUDE-of-MAGNITUDES, CENTRE-
	 * of-CENTRES) cross-axis structural-endpoint corner: S266
	 * supplies the CENTRE-of-CENTRES scalar, S267 supplies the
	 * MAGNITUDE-of-MAGNITUDES scalar, completing the cross-axis
	 * collapse of the (audible-endpoint, rest-endpoint) x
	 ...
	 * Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, and for the all-collapse
	 * case where both endpoint MAGNITUDE accessors return 0 (a
	 * single-audible-step AND single-rest-step melody, or a
	 * no-audible-notes AND no-rests melody, neither of which
	 * exists in today's catalogue but both of which are handled
	 * defensively). Returns the shared value for any chime whose
	 * audible-endpoint MAGNITUDE and rest-endpoint MAGNITUDE
	 * happen to be equal (the MAGNITUDE-of-MAGNITUDES of two
	 * equal scalars is that same scalar). When only one axis has
	 * structural spread (e.g. a flat-duration audible envelope
	 * where `durationSpanMs` collapses to 0), the MAGNITUDE-of-
	 * MAGNITUDES is exactly half of the non-zero endpoint
	 * MAGNITUDE -- this is the documented defensive behaviour,
	 * not a bug, and reflects the fact that the collapsed axis
	 * genuinely contributes a 0-ms structural MAGNITUDE to the
	 * cross-axis mean. Profile-state INDEPENDENT: the
	 * catalogued cross-axis endpoint MAGNITUDE-of-MAGNITUDES is
	 * the same on SILENT / MEETING profiles as on GENERAL /
	 * OUTDOOR / HEADSET. Distinct from `durationSpanMs`
	 * (audible-axis structural-endpoint MAGNITUDE only, not the
	 * cross-axis collapse), distinct from
	 * `restDurationEndpointSpanMs` (rest-axis structural-
	 ...
	 * `firstNoteDurationMs`, one reverse via `lastNoteDurationMs`,
	 * one forward via `firstRestDurationMs`, one reverse via
	 * `lastRestDurationMs`, all reused through the S254 / S263
	 * accessors) plus two unsigned additions and one divide-by-
	 * two; no engine interaction, no persisted state, no per-
	 * call allocation. Uses `uint32_t` for the intermediate sum
	 * so the audible-MAGNITUDE + rest-MAGNITUDE addition never
	 * wraps even at the full `uint16_t` ceiling. Header surface
	 * grows by exactly one public symbol (`static uint16_t
	 * meanEndpointSpanMs`); the cpp adds a single function next
	 * to the existing `count` / `valid` / `name` / `melody` /
	 * `play` / `tryPlay` / `isSilenced` / `durationMs` /
	 * `noteCount` / `firstFreqHz` / `lastFreqHz` / `gapMs` /
	 * `loops` / `silhouette` / `pitchSpanHz` / `peakFreqHz` /
	 ...
	 * asset cost. Every existing call site of the catalogue
	 * keeps byte-identical behaviour -- the new helper is purely
	 * additive. Together with S266 this closes the (MAGNITUDE-
	 * of-MAGNITUDES, CENTRE-of-CENTRES) cross-axis structural-
	 * endpoint corner on the duration axis.
	 */
	static uint16_t meanEndpointSpanMs(uint8_t id);
	/**
	 * S268 -- Derived cross-axis CEILING-FLOOR envelope MAGNITUDE-
	 * of-MAGNITUDES accessor on the DURATION axis for chime `id`.
	 * Returns the arithmetic mean, in ms, of the audible-axis
	 ...
	 * `meanEndpointSpanMs(id)` (S267) which collapses the same
	 * pair on the structural-endpoint axis. Where S267 closes the
	 * cross-axis MAGNITUDE-of-MAGNITUDES corner on the structural-
	 * endpoint axis (bookend FIRST/LAST per-axis duration spread),
	 * S268 closes the cross-axis MAGNITUDE-of-MAGNITUDES corner on
	 * the CEILING-FLOOR envelope axis (peak/trough per-axis
	 * duration spread). A future PhoneDiagScreen "Sound test"
	 * walk that wants to caption the cross-axis CEILING-FLOOR
	 * envelope duration spread of a cue, or a "Settings -> Sounds
	 * -> System chimes" picker row caption like "(envelope span
	 * 80 ms)" can read a dedicated derived accessor for the
	 * cross-axis envelope MAGNITUDE-of-MAGNITUDES instead of
	 * computing `(audibleDurationSpanMs(id) +
	 * restDurationSpanMs(id)) / 2` at the call site.
	 *
	 * Returns 0 for an out-of-range id, for the (currently
	 * impossible) empty-melody case, and for the all-collapse
	 * case where both envelope MAGNITUDE accessors return 0 (a
	 * single-audible-step AND single-rest-step melody, or a
	 * no-audible-notes AND no-rests melody, neither of which
	 * exists in today's catalogue but both of which are handled
	 * defensively). Returns the shared value for any chime whose
	 * audible-envelope MAGNITUDE and rest-envelope MAGNITUDE
	 * happen to be equal (the MAGNITUDE-of-MAGNITUDES of two
	 * equal scalars is that same scalar). When only one axis has
	 * envelope spread (e.g. a flat-duration audible envelope
	 * where `audibleDurationSpanMs` collapses to 0), the
	 * MAGNITUDE-of-MAGNITUDES is exactly half of the non-zero
	 * envelope MAGNITUDE -- documented defensive behaviour,
	 ...
	 * profiles as on GENERAL / OUTDOOR / HEADSET. Distinct from
	 * `audibleDurationSpanMs` (audible-axis CEILING-FLOOR
	 * envelope MAGNITUDE only, not the cross-axis collapse),
	 * distinct from `restDurationSpanMs` (rest-axis CEILING-FLOOR
	 * envelope MAGNITUDE only, not the cross-axis collapse),
	 * distinct from `meanEndpointSpanMs` (cross-axis structural-
	 * endpoint MAGNITUDE-of-MAGNITUDES on the FIRST/LAST bookend
	 * axis, not the peak/trough envelope axis), distinct from
	 * `meanEndpointDurationMs` (cross-axis structural-endpoint
	 * CENTRE-of-CENTRES, not envelope MAGNITUDE-of-MAGNITUDES),
	 * distinct from `durationSpanMs` / `restDurationEndpointSpanMs`
	 * (per-axis structural-endpoint MAGNITUDE accessors, not
	 * envelope), distinct from `pitchSpanHz` / `audiblePitchSpanHz`
	 * (pitch-axis MAGNITUDE accessors, not duration-axis).
	 *
	 * Cheap O(notes): two linear scans (one peak/trough sweep
	 * via `audibleDurationSpanMs` -- itself a single pass through
	 * `peakNoteDurationMs` / `troughNoteDurationMs`, and a
	 * matching one via `restDurationSpanMs` through
	 * `peakRestDurationMs` / `troughRestDurationMs`, all reused
	 * through the S256 / S260 accessors) plus two unsigned
	 * additions and one divide-by-two; no engine interaction, no
	 * persisted state, no per-call allocation. Uses `uint32_t`
	 * for the intermediate sum so the audible-MAGNITUDE + rest-
	 * MAGNITUDE addition never wraps even at the full `uint16_t`
	 * ceiling. Header surface grows by exactly one public symbol
	 * (`static uint16_t meanEnvelopeSpanMs`); the cpp adds a
	 * single function next to the existing `count` / `valid` /
	 * `name` / `melody` / `play` / `tryPlay` / `isSilenced` /
	 * `durationMs` / `noteCount` / `firstFreqHz` / `lastFreqHz`
	 * / `gapMs` / `loops` / `silhouette` / `pitchSpanHz` /
	 ...
	 * / `meanEndpointDurationMs` / `meanEndpointSpanMs` cluster.
	 * No new includes, no new const data, no new SPIFFS asset
	 * cost. Every existing call site of the catalogue keeps
	 * byte-identical behaviour -- the new helper is purely
	 * additive. Together with S267 this lifts the cross-axis
	 * MAGNITUDE-of-MAGNITUDES from the structural-endpoint axis
	 * to also covering the CEILING-FLOOR envelope axis,
	 * completing the (endpoint, envelope) x (MAGNITUDE-of-
	 * MAGNITUDES) duration-axis pair; the natural follow-up is
	 * the matching envelope-axis CENTRE-of-CENTRES (arithmetic
	 * mean of `meanNoteDurationMs(id)` and `meanRestDurationMs(id)`)
	 * to close the (endpoint, envelope) x (MAGNITUDE, CENTRE)
	 * cross-axis 2x2 grid.
	 */
