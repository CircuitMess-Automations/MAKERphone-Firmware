#include "PhoneOwnerNameScreen.h"
...
// the owner-name editor slots in beside PhoneScratchpad (S140), PhoneNotepad
// (S64) and the rest of the SYSTEM screens without a visual seam. Inlined per
...
//   y = 12        caption "OWNER NAME" (pixelbasic7, ~7 px tall)
...
//
// Mirrors PhoneScratchpad's layout 1:1 so the two T9-entry screens read
...
PhoneOwnerNameScreen::PhoneOwnerNameScreen() : LVScreen() {
	// Full-screen container, no scrollbars, no padding - same blank-canvas
	// pattern PhoneScratchpad / PhoneNotepad / PhoneSettingsScreen use.
	...
	// Pre-load the persisted owner name so the screen opens with the
	...
	if(t9Input) {
		const char* persisted = Settings.get().ownerName;
...
PhoneOwnerNameScreen::~PhoneOwnerNameScreen() = default;

void PhoneOwnerNameScreen::onStart() {
	Input::getInstance()->addListener(this);
}

void PhoneOwnerNameScreen::onStop() {
	Input::getInstance()->removeListener(this);
}

// ---------- accessors ----------------------------------------------------

uint16_t PhoneOwnerNameScreen::getLength() const {
	if(t9Input == nullptr) return 0;
	return (uint16_t) t9Input->getText().length();
}

const char* PhoneOwnerNameScreen::getText() const {
	if(t9Input == nullptr) return "";
	// Returning a c_str pointer into a temporary String would be UB,
	// so we cache via a static thread-local-safe-enough buffer; the
	// firmware is single-threaded for the UI loop so a per-instance
	// dance is overkill. PhoneScratchpad / PhoneNotepad use the same
	// "lower-case const char*" pattern.
...
void PhoneOwnerNameScreen::buildView() {
	// Caption -- "OWNER NAME". Painted in cyan to match the section
	...
	lv_label_set_text(captionLabel, "OWNER NAME");
	...
	lv_obj_set_pos(t9Input->getLvObj(),
				   ...
				   kT9Y);
	t9Input->setPlaceholder("YOUR NAME");
	// First-letter-upper fits the "Albert"-on-the-lock-screen feel.
...
void PhoneOwnerNameScreen::refreshCaption() {
	if(captionLabel == nullptr) return;
	lv_label_set_text(captionLabel, "OWNER NAME");
}

void PhoneOwnerNameScreen::refreshSoftKeys() {
	if(softKeys == nullptr) return;
	softKeys->setLeft("CLEAR");
	softKeys->setRight("DONE");
}

void PhoneOwnerNameScreen::refreshCharCounter() {
...
void PhoneOwnerNameScreen::refreshDirty() {
...
void PhoneOwnerNameScreen::persist() {
	if(t9Input == nullptr) return;
	...
	// last letter. Same gesture PhoneScratchpad / PhoneNotepad use.
	...
	char* slot = Settings.get().ownerName;
	memset(slot, 0, sizeof(Settings.get().ownerName));
...
void PhoneOwnerNameScreen::clearBuffer() {
	if(t9Input != nullptr) {
		t9Input->clear();
	}
	...
	// the change to NVS.
	memset(Settings.get().ownerName, 0, sizeof(Settings.get().ownerName));
	Settings.store();
...
void PhoneOwnerNameScreen::onClearPressed() {
	if(softKeys) softKeys->flashLeft();
	clearBuffer();
}

void PhoneOwnerNameScreen::onDonePressed() {
...
void PhoneOwnerNameScreen::buttonPressed(uint i) {
	// Always-edit mode: route digits + bumpers through the T9 funnel
	// the same way PhoneScratchpad / PhoneNotepad do.
	switch(i) {
...
void PhoneOwnerNameScreen::buttonHeld(uint i) {
	if(i == BTN_BACK) {
...
void PhoneOwnerNameScreen::buttonReleased(uint i) {
	if(i == BTN_BACK) {
